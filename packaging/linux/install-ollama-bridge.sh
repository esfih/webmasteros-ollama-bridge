...
INSTALL_ROOT="${HOME}/.local/share/webmasteros/ollama-bridge"
CONFIG_ROOT="${HOME}/.config/webmasteros/ollama-bridge"
...
cat > "${INSTALL_ROOT}/run-ollama-bridge.sh" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PYTHON_CMD}" "${INSTALL_ROOT}/bridge.py" --config "${CONFIG_PATH}"
EOF
chmod +x "${INSTALL_ROOT}/run-ollama-bridge.sh"

echo "Installed WebmasterOS Ollama Bridge to ${INSTALL_ROOT}"
echo "Config path: ${CONFIG_PATH}"
echo "Run with: ${INSTALL_ROOT}/run-ollama-bridge.sh"
